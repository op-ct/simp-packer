...
Puppet::Functions.create_function(:'nested_lookup::yaml_key') do
  ...
  def yaml_data(key, options, context, context_key = :'nested_lookup::ppyaml')
    ...
    Puppet.override({context_key => [invocation, next_level]}, "Protect against recursive lookup/eval of sae key") do
      recursion_check(invocation, level, next_level, key) do
        ...
        Puppet.notice(">>>> value class: '#{value.class}'")
        Puppet.notice("@@@@@@@@@>>>> invocation: '#{invocation}' #{invocation.class}'")
        if value.is_a? String
          value = context.interpolate(value)

        elsif  value.is_a? Hash
          value = value.map do |k,v|
            Puppet.notice(">>>> value.keys: '#{k}': '#{v}'")
            Puppet.notice(">>>> value.keys(i): '#{k}': '#{context.interpolate(v).class}'")
            [k,context.interpolate(v)]
          end.to_h
          keys_to_merge_at_top_level = value.select{|k,v| k =~ /\A@@MERGE_KEYS/ }.keys
          keys_to_merge_at_top_level.each do |key|
            value = value.delete(key).merge(value)
          end
        end


        Puppet.info(">>>> value: '#{value}'")
  ...
  def do_pp_interpolation(value)
    ...
    when Hash
      ...
      result
    when String
      call_function('nested_lookup::eval', value)
    else
      value
    end
  end
